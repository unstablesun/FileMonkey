...
public class testLzmaWebGL : MonoBehaviour
{
#if UNITY_WEBGL

	private string myFile = "testLZ4b.png.lzma";
	...
	void Start(){
		...
		//get an lzma file from StreamingAssets
		#if !UNITY_EDITOR
	...
	void OnGUI(){
		...
		if (downloadDone2 == true) {
			...
			if (GUI.Button(new Rect(10, 90, 230, 50), "start StreamingAssets lzma test")) {
				#if !UNITY_EDITOR
					plog("lzma: "+lzma.decompressBuffer(ww2.bytes, ref outbuffer).ToString());
				#else
					plog("lzma: "+lzma.decompressBuffer(wwb, ref outbuffer).ToString());
				#endif
