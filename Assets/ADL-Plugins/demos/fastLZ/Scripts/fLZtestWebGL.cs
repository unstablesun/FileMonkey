...
public class fLZtestWebGL : MonoBehaviour
{
#if UNITY_WEBGL

	private string myFile = "testLZ4b.png.flz";
	...
	void Start(){
		...
		//get an flz file as saved buffer from StreamingAssets
		#if !UNITY_EDITOR
	...
	void OnGUI(){
		...
		if (downloadDone2 == true) {
			...
			if (GUI.Button(new Rect(10, 90, 230, 50), "start StreamingAssets flz test")) {
				#if !UNITY_EDITOR
					plog("flz decompress: "+fLZ.decompressBuffer(ww2.bytes, ref outbuffer).ToString() );
				#else
					plog("flz decompress: "+fLZ.decompressBuffer(wwb, ref outbuffer).ToString() );
				#endif
